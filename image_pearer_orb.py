...
match_threshold = .25 # Define match threshold. Up to 1.0. Higher = stricter requirements to create an image pair. You may have to adjust depending on the source.
...
prepend_string = "orb_" # Define a string to prepend to image name. For example, putting "blah" would result in output images such as "blah_000001.png".
...
for lr_index, lr_image in enumerate(lr_images):
    ...
    # Compute ORB keypoints and descriptors for LR image
    lr_kp, lr_des = orb.detectAndCompute(lr_image_data, None)
    ...
    for i in range(-num_images, num_images + 1):
        ...
        # Compute ORB keypoints and descriptors for HR image
        hr_kp, hr_des = orb.detectAndCompute(hr_image_data, None)
        ...
        if hr_kp and hr_des is not None:
            ...
            try:
                ...
                # If ORB failed to locate keypoints for the LR image, print a message and move onto the next image
                # print(f"ORB failed to locate keypoints for {lr_image}.")
                continue
            ...
            print(f"ORB failed to locate keypoints for {hr_path}/{hr_images[hr_index + i]}. Added to blacklist.")
...
print(f"ORB was unable to create keypoints for the following images from {hr_path}, and excluded them for pairing: {blacklist}")